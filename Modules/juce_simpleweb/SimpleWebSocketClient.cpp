...
{
	this->isClosing = true;
	signalThreadShouldExit(); //a thread that has not started its io loop yet no longer starts it

	stopInternal();

	if (Thread::getCurrentThreadId() != this->getThreadId())
	{
		//A stop that reaches the io loop before it runs is lost (the loop is created on the thread, right after start()),
		//and the thread was then killed after 1 s, inside asio : asked again until the thread has left
...
void SimpleWebSocketClient::send(const String& message)
{
	...
	{
		const ScopedLock sl(wsLock);
		c = this->connection;
	}
	if (c != nullptr) c->send(message.toStdString());
}
...
void SimpleWebSocketClient::send(const char* data, int numData)
{
	...
	if (c != nullptr) c->send(out_message, nullptr, 130);
}
...
void SimpleWebSocketClient::stopInternal()
{
	...
	if (c != nullptr) c->send_close(1000, "Time to split my friend");
	stopIO();
}

void SimpleWebSocketClient::stopIO()
{
	const ScopedLock sl(wsLock);
...
void SimpleWebSocketClient::initWS()
{
	{
		const ScopedLock sl(wsLock);
		if (threadShouldExit()) return; //stopped before the loop started
	...
	ws->start(); //ws is only replaced here, on this thread
}
...
SecureWebSocketClient::~SecureWebSocketClient()
{
	//here, while ws still exists : the base destructor's stop() only reaches the base's empty stopInternal(), so the io
	//thread went on inside a destroyed client until it was killed
	stop();
}

void SecureWebSocketClient::send(const String& message)
{
	std::shared_ptr<WssClient::Connection> c;
	{
		const ScopedLock sl(wsLock);
		c = this->connection;
	}
...
void SecureWebSocketClient::stopIO()
{
	const ScopedLock sl(wsLock);
	if (ws != nullptr) ws->stop();
}
