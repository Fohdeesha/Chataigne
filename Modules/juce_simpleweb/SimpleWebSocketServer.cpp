...
{
	const ScopedLock sl(connectionMap.getLock()); //closed connections are removed on the io thread
	HashMap<String, std::shared_ptr<WsServer::Connection>, DefaultHashFunctions, CriticalSection>::Iterator it(connectionMap);
	while (it.next())
	{
		if (simplewebDropIfStalled(it.getValue())) continue;
...
{
	const ScopedLock sl(connectionMap.getLock()); //the io thread removes closed connections
	if (connectionMap.contains(id))
	{
		if (auto connection = connectionMap[id]) if (!simplewebDropIfStalled(connection)) connection->send(message.toStdString());
	}
	else
	{
...
void SimpleWebSocketServer::sendTo(const MemoryBlock& data, const String& id)
{
	...
	if (connectionMap.contains(id))
	{
		if (auto connection = connectionMap[id]) if (!simplewebDropIfStalled(connection)) connection->send(out_message, nullptr, 130); // 130 = binary
	}
	else
	{
		DBG("Websocket connection not found : " << id);
...
{
	const ScopedLock sl(connectionMap.getLock());
	if (!connectionMap.contains(id))
	{
		return;
	}
	if (auto connection = connectionMap[id]) connection->send_close(code, reason.toStdString());
}
...
{
	const ScopedLock sl(connectionMap.getLock()); //closed connections are removed on the io thread
	HashMap<String, std::shared_ptr<WssServer::Connection>, DefaultHashFunctions, CriticalSection>::Iterator it(connectionMap);
	while (it.next())
	{
		if (simplewebDropIfStalled(it.getValue())) continue;
		it.getValue()->send(message.toStdString());
