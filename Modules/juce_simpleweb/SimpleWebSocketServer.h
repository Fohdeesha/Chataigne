...
//A client that stopped reading is dropped instead of being queued for without end (the web dashboard and the OSCQuery
//feedback push every change, 240 a second at 240 fps) : it reconnects and gets the data again.
static constexpr size_t SIMPLEWEB_MAX_PENDING_SENDS = 4096;

template<class ConnectionPtr> static bool simplewebDropIfStalled(const ConnectionPtr& connection)
{
	if (connection == nullptr || connection->pending_sends() < SIMPLEWEB_MAX_PENDING_SENDS) return false;
...
public:
	...
	std::shared_ptr<asio::io_service> ioService;
	juce::HashMap<juce::String, std::shared_ptr<WssServer::Connection>, juce::DefaultHashFunctions, juce::CriticalSection> connectionMap;
