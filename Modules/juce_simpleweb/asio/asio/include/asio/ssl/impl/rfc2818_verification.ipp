    ...
    bool preverified, verify_context& ctx) const
{
  ...
  for (int i = 0; i < sk_GENERAL_NAME_num(gens); ++i)
  {
    ...
    if (gen->type == GEN_DNS && !is_address)
    {
      ...
      const unsigned char* domain_data = ASN1_STRING_get0_data(domain);
      const int domain_length = ASN1_STRING_length(domain);
      if (ASN1_STRING_type(domain) == V_ASN1_IA5STRING
          && domain_data && domain_length > 0)
      {
        const char* pattern = reinterpret_cast<const char*>(domain_data);
        std::size_t pattern_length = static_cast<std::size_t>(domain_length);
    ...
    else if (gen->type == GEN_IPADD && is_address)
    {
      ...
      const unsigned char* ip_address_data =
        ASN1_STRING_get0_data(ip_address);
      const int ip_address_length = ASN1_STRING_length(ip_address);
      if (ASN1_STRING_type(ip_address) == V_ASN1_OCTET_STRING
          && ip_address_data)
      {
        if (address.is_v4() && ip_address_length == 4)
        {
          ip::address_v4::bytes_type bytes = address.to_v4().to_bytes();
          if (memcmp(bytes.data(), ip_address_data, 4) == 0)
          {
            GENERAL_NAMES_free(gens);
            return true;
          }
        }
        else if (address.is_v6() && ip_address_length == 16)
        {
          ip::address_v6::bytes_type bytes = address.to_v6().to_bytes();
          if (memcmp(bytes.data(), ip_address_data, 16) == 0)
          {
            GENERAL_NAMES_free(gens);
            return true;
          }
  ...
  if (common_name)
  {
    const unsigned char* common_name_data =
      ASN1_STRING_get0_data(common_name);
    const int common_name_length = ASN1_STRING_length(common_name);
    if (common_name_data && common_name_length > 0)
    {
      const char* pattern =
        reinterpret_cast<const char*>(common_name_data);
      std::size_t pattern_length =
        static_cast<std::size_t>(common_name_length);
      if (match_pattern(pattern, pattern_length, host_.c_str()))
        return true;
    }
  }
