	...
	sequenceNotifier(10)
{
	...
	//unitSteps gives the time label its frame display and the timeline its click snapping, but the playhead itself
	//must not be rounded to the frame grid : the play loop sets it from the wall clock, and rounding it made every
	//value the layers evaluate carry up to half a frame of time error (a periodic velocity ripple on any output).
	currentTime->snapToUnitSteps = false;
...
void Sequence::onContainerParameterChangedInternal(Parameter* p)
{
	...
	else if (p == isPlaying)
	{
		...
		if (isPlaying->boolValue())
		{
			...
			prevTime = currentTime->floatValue();
			//the play thread is the show's metronome : above the UI and the message thread, below audio
			if (!isThreadRunning()) startThread(Priority::high);
		}
...
void Sequence::run()
{
	...
	while (!threadShouldExit())
	{
		...
		//A stop, pause or the finish issued from this thread (a command in a mapping output, the finish trigger below)
		//only reaches isPlaying's handler later, on the message thread : until then this loop kept moving the time, past
		//a Stop's 0 or a Pause's position, and fired Finish again every frame (a Conductor's Auto Next skipped cues).
		//Checked under the time lock that setCurrentTime() takes, so a Stop's jump to 0 is never overwritten.
		bool timeSet = false;
		{
			GenericScopedLock lock(sequenceTimeLock);
			if (!isPlaying->boolValue()) break;
		...
		if (timeSet)
		{
			playThreadTick(currentTime->floatValue());
			if (!isPlaying->boolValue()) break;
		}
		...
		//Sleep until the next slot of a fixed grid anchored at loop start. The slot comes from the elapsed time, so a
		//late wake-up or a slow iteration skips slots instead of shifting the grid and the average rate stays exactly
		//fps. Waiting "one cycle minus what this iteration took" instead rounded every wait up to a whole millisecond
		//and added the wake-up latency on top of it, which made a 60 fps sequence run at about 55 Hz.
		double millisPerCycle = 1000.0 / fps->floatValue();
		double elapsed = Time::getMillisecondCounterHiRes() - loopStartMillis;
		double nextSlot = (floor(elapsed / millisPerCycle) + 1) * millisPerCycle;
		wait(nextSlot - elapsed);
	}
}

void Sequence::playThreadTick(float time)
{
	//Only called from run(). The tick lock keeps the list stable (add/remove take it) without touching the list's own
	//lock, see the header ; a copy is walked so a listener registering another one from its tick cannot disturb the pass.
	const ScopedLock sl(playThreadTickLock);
	Array<SequenceListener*> listeners;
	for (auto* l : sequenceListeners.getListeners()) listeners.add(l);
	for (auto* l : listeners) l->sequencePlayThreadTick(this, time);
}
