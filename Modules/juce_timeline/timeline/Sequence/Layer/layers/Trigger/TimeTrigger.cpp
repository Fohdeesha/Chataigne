...
void TimeTrigger::setMovePositionReferenceInternal()
{
	...
	lengthMoveReference = length->floatValue();
}
...
{
	//An Alt-drag over a selection scales lengths as well as positions. Added first so an undo restores
	//the length before the time; only when it actually changed, so plain moves stay at two actions.
	if (length->floatValue() != lengthMoveReference) actions.add(length->setUndoableValue(lengthMoveReference, length->floatValue(), true));
