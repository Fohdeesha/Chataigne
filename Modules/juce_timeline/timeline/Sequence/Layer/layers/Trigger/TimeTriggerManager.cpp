...
{
	//Sequence calls this before setCurrentTime(), so clearing the flags here used to swallow the
	//release of any bar still held at the loop point : the rewind branch of sequenceCurrentTimeChanged
	//then found isTriggered already false and did nothing. A looping sequence left notes hanging.
	releaseHeldBars();
