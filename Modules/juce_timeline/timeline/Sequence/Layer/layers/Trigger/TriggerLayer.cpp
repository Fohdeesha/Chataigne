...
{
	if (ttm == nullptr) return;
	float totalTime = sequence->totalTime->floatValue();
	for (auto& i : ttm->items)
	{
		arrayToFill->addIfNotAlreadyThere(i->time->floatValue());

		//a bar's end is a snap target too, the same way LayerBlockManager publishes block ends.
		//Clamped because length is not capped at the sequence end.
		float length = i->length->floatValue();
		if (length > 0) arrayToFill->addIfNotAlreadyThere(jmin(i->time->floatValue() + length, totalTime));
	}
}
