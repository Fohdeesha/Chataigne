...
namespace
{
	constexpr double audioLoopEndTolerance = 0.03;

	//The audio device's thread renders each block holding the graph's callback lock (AudioProcessorPlayer takes it). What
	//a block reads - the layer behind the processor, the current clip, the processor's and clips' buffers - can be
	//replaced or freed while holding it. Graph topology calls (add/remove node, connections) stay outside it.
	struct ScopedAudioCallbackLock
	{
		explicit ScopedAudioCallbackLock(AudioProcessorGraph* graph) : lock(graph != nullptr ? &graph->getCallbackLock() : nullptr)
		{
			if (lock != nullptr) lock->enter();
		}
		~ScopedAudioCallbackLock()
		{
...
void AudioLayer::clearItem()
{
	...
	BaseItem::clearItem();
	setAudioProcessorGraph(nullptr); //the processor lets go of the layer first, so the clips can go
	currentClip = nullptr;
	clipManager.clear();
...
void AudioLayer::setAudioProcessorGraph(AudioProcessorGraph* graph, AudioProcessorGraph::NodeID outputGraphID)
{
	...
	if (currentGraph != nullptr)
	{
		...
		{
			//the processor keeps being rendered until the graph's next block picks up the rebuilt sequence : it must not
			//be halfway through a block that reads the layer when it lets go of it
			ScopedAudioCallbackLock sl(currentGraph);
	...
	if (currentGraph != nullptr)
	{
		...
		//The graph picks the id (one above every id it holds). A counter of our own collided with Play Audio File's once
		//enough layers had been set up, and addNode then refused the node and destroyed the processor we still pointed to.
		auto proc = std::unique_ptr<AudioLayerProcessor>(createAudioLayerProcessor());
		AudioLayerProcessor* newProcessor = proc.get();
		if (AudioProcessorGraph::Node::Ptr node = currentGraph->addNode(std::move(proc)))
		{
...
void AudioLayer::updateCurrentClip()
{
	...
	{
		ScopedAudioCallbackLock sl(currentGraph); //the device's thread copies it at the start of every block
		currentClip = target;
	}
...
void AudioLayer::itemsRemoved(Array<LayerBlock*> clips)
{
	for (auto& clip : clips)
	{
		...
		releaseIfCurrentClip(clip);
	}

	updateCurrentClip();
}

void AudioLayer::resyncClipToTimeline()
{
	if (currentClip == nullptr || !sequence->isPlaying->boolValue()) return;
...
void AudioLayer::releaseIfCurrentClip(LayerBlock* clip)
{
	//The clip is deleted right after this notification. Under the playhead it would still be "in range", so
	//updateCurrentClip() kept it current and the device's thread went on rendering a clip about to be freed.
	if (currentClip.get() != clip) return;
	ScopedAudioCallbackLock sl(currentGraph);
	currentClip = nullptr;
}
...
void AudioLayer::updatePlayConfigDetails()
{
	...
	//prepareToPlay reallocates the declick buffers the device's thread writes in every block
	ScopedAudioCallbackLock sl(currentGraph);
...
void AudioLayer::updateClipConfig(AudioLayerClip* clip, bool updateOutputChannelRemapping)
{
	...
	ScopedAudioCallbackLock sl(currentGraph); //a playing clip is being rendered : its sources are reset below
...
void AudioLayer::onControllableFeedbackUpdateInternal(ControllableContainer* cc, Controllable* c)
{
	...
	else if (currentClip != nullptr)
	{
		...
		else if (c == currentClip->time || c == currentClip->stretchFactor)
		{
			...
			if (c == currentClip->stretchFactor && currentGraph != nullptr)
			{
				ScopedAudioCallbackLock sl(currentGraph);
...
void AudioLayer::loadJSONDataInternal(var data)
{
	...
	//The output channels cannot be set up while the file loads (updateSelectedOutChannels returns), and nothing else
	//sets them up afterwards : the Sound Card applies its device during the load and does not rebuild for an unchanged
	//setup. Left at none, the layer renders no audio and every stop waits a second for a block that never pulls the clip.
...
{
	if (currentClip != nullptr && currentGraph != nullptr) //no graph without an audio module
	{
		ScopedAudioCallbackLock sl(currentGraph);
		currentClip->setPlaySpeed(sequence->playSpeed->doubleValue());
		currentClip->prepareToPlay(currentGraph->getBlockSize(), currentGraph->getSampleRate());
