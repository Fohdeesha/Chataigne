...
public:
	...
	*/
	...
	juce::OSCBundle readBundle(size_t maxBytesToRead = std::numeric_limits<size_t>::max())
	{
		...
		//same cap as JUCE's own parser : one datagram can nest thousands of levels and overflow the stack
		if (bundleDepth >= maxBundleDepth) throw juce::OSCFormatError("OSC input stream format error: bundles nested too deeply");
		struct DepthGuard
		{
			explicit DepthGuard(int& d) : depth(d) { ++depth; }
			~DepthGuard() { --depth; }
			int& depth;
		} depthGuard(bundleDepth);
...
private:
	...
	int bundleDepth = 0;
	static constexpr int maxBundleDepth = 32;
