...
public:
	...
	//Calls f for every message of the bundle, descending into nested bundles up to maxDepth levels (deeper ones are dropped).
	//OSCBundle::Element::getMessage() throws on an element that is itself a bundle, which on the message thread ends the process.
	static void forEachMessageInBundle(const juce::OSCBundle& b, const std::function<void(const juce::OSCMessage&)>& f, int maxDepth = 16);
