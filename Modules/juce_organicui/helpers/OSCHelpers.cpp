...
void OSCHelpers::handleControllableForOSCMessage(Controllable* c, const OSCMessage& m, int dataOffset)
{
	...
	else if (Parameter* p = dynamic_cast<Parameter*>(c))
	{
		...
		case Controllable::COLOR:
			//check the size before reading any argument : a message with none must not read past the end
			if (m.size() > dataOffset && m[dataOffset].isColour()) static_cast<ColorParameter*>(p)->setColor(getColourFromOSC(m[dataOffset].getColour()));
			else if (m.size() < dataOffset + 3) LOG("Parameter " << p->niceName << " requires a colour or at least 3 arguments");
			else static_cast<ColorParameter*>(p)->setColor(Colour::fromFloatRGBA(getFloatArg(m[dataOffset]), getFloatArg(m[dataOffset + 1]), getFloatArg(m[dataOffset + 2]), m.size() >= dataOffset + 4 ? getFloatArg(m[dataOffset + 3]) : 1));
			break;
...
void OSCHelpers::forEachMessageInBundle(const OSCBundle& b, const std::function<void(const OSCMessage&)>& f, int maxDepth)
{
	for (auto& e : b)
	{
		if (e.isMessage()) f(e.getMessage());
		else if (e.isBundle() && maxDepth > 0) forEachMessageInBundle(e.getBundle(), f, maxDepth - 1);
	}
}
