...
{
	data.add(new WatcherData(Time::getMillisecondCounterHiRes() / 1000.0, val)); //not the 32-bit counter : it wraps every 49.7 days
}
...
void ControllableDetectiveWatcher::run()
{
	while (!threadShouldExit() && !controllable.wasObjectDeleted())
	{
		...
		{
			//under the list's lock, which the interface holds while it draws from the list and oldestVal : this pass
			//deleted entries and replaced oldestVal under it
			const ScopedLock sl(data.getLock());
