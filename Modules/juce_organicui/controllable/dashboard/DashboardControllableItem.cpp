...
{
	//guard on the reference we are about to dereference, not on the parallel one: controllable is its
	//own WeakReference and is the only thing that says whether this listener still has an owner
	if (controllable != nullptr && !controllable.wasObjectDeleted()) controllable->removeControllableListener(this);
