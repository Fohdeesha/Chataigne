...
ControllableUI* DashboardTriggerItemUI::createControllableUI()
{
	...
	if (f.existsAsFile() && triggerItem->trigger != nullptr)
	{
...
void DashboardTriggerItemUI::updateUIParametersInternal()
{
	...
	//the target can be re-resolved to something that is not a trigger : the UI is then that thing's, not a TriggerUI
	TriggerUI* tui = dynamic_cast<TriggerUI*>(itemUI.get());
