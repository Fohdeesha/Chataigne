...
{
	if (EnumParameter* ep = dynamic_cast<EnumParameter*>(parameter.get())) ep->removeEnumParameterListener(this);
}

void DashboardEnumParameterItem::setInspectableInternal(Inspectable* i)
{
	//The option listener follows the target : removing the item (or undoing its creation) clears the target in
	//clearItem() before the destructor runs, and the enum then called into the deleted item the next time its options
	//changed (openHAB adds options at runtime). The constructor adds the first one, the base constructor's call to
	//this does not reach this override.
	if (EnumParameter* ep = dynamic_cast<EnumParameter*>(parameter.get())) ep->removeEnumParameterListener(this);
	DashboardParameterItem::setInspectableInternal(i);
	if (EnumParameter* ep = dynamic_cast<EnumParameter*>(parameter.get())) ep->addEnumParameterListener(this);
}
