...
//Lock order : Parameter::setValue() takes valueSetLock, then setValueInternal() takes the options lock. So nothing here
//calls setValue() or a listener while holding the options lock : a thread doing that while another sets the value (a
//sequence's play thread setting it while the UI, OSC or a binding does) deadlocked both, the second one on a spin lock.
EnumParameter* EnumParameter::addOption(String key, var data, bool selectIfFirstOption)
{
	bool selectIt = false;
	{
		...
		selectIt = enumValues.size() == 1 && selectIfFirstOption;
...
void EnumParameter::removeOption(String key)
{
	{
		GenericScopedLock lock(enumValues.getLock());
		const int index = getIndexForKey(key);
		if (index < 0) return;
...
void EnumParameter::setOptions(Array<EnumValue> options)
{
	...
	for (;;)
	{
		...
		{
			GenericScopedLock lock(enumValues.getLock());
			if (enumValues.size() <= options.size()) break;
			lastKey = enumValues.getLast()->key;
		}
		removeOption(lastKey);
	}
}
...
String EnumParameter::getValueKey() {
	GenericScopedLock lock(enumValues.getLock()); //written under it too, see setValueInternal()
...
bool EnumParameter::setValueWithData(var data, bool silentSet, bool force, bool forceOverride)
{
	...
	setValueWithKey(key, silentSet, force, forceOverride);
	return true;
}
...
bool EnumParameter::setValueAtIndex(int index, bool silentSet, bool force, bool forceOverride)
{
	...
	{
		GenericScopedLock lock(enumValues.getLock());
		if (index < 0 || index >= enumValues.size()) return false;
		key = enumValues[index]->key;
	}
	setValueWithKey(key, silentSet, force, forceOverride);
	return true;
}
...
void EnumParameter::setPrev(bool loop, bool addToUndo)
{
	String oldValue, newValue;
	{
		...
		int targetIndex = getIndexForKey(oldValue) - 1;
		if (targetIndex < 0)
		{
			if (loop) targetIndex = enumValues.size() - 1;
			else return;
		}
		newValue = enumValues[targetIndex]->key;
	}

	if (addToUndo) setUndoableValue(oldValue, newValue);
...
{
	//Called by Parameter::setValue() holding valueSetLock : the options lock comes second, and the key is written under
	//it so getValueKey() / getValueData(), which take only the options lock, never read it halfway through a write
	GenericScopedLock lock(enumValues.getLock());
