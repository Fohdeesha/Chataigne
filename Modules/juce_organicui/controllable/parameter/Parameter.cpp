...
Parameter::~Parameter()
{
	...
	//A Go to Value fading this parameter runs on the interpolator's thread : it is either done with it, or sees it
	//deleted from here on (clearing the reference only at the end of this destructor, or from inspectableDestroyed,
	//which comes after the members are gone, left a window where the thread set a parameter being destroyed)
	if (ValueInterpolator::Manager* m = ValueInterpolator::Manager::getInstanceWithoutCreating())
	{
		const ScopedLock interpLock(m->interpLock);
		Parameter::masterReference.clear();
	}

	//Members die in reverse order : queuedNotifier and notifyLock (declared last) before automation and expression. The
	//automation's thread sets this parameter, and the expression updates it : both stop before anything of it dies.
	if (automation != nullptr) automation->stopThread(1000);
	expression.reset();
...
void Parameter::notifyValueChanged() {
	...
	{
		//Always post, and never touch notifyLock here : the message thread holds it for a whole synchronous listener
		//chain, and giving up when it was busy lost the change for good (the value was stored, nothing ever notified
		//it, and setting the same value again returns early).
		WeakReference<Parameter> safeThis(this);
...
//Called from any thread firing a Go to Value, while run() updates the interpolators : the lookup, the update of a
//running one and the add all happen under interpLock (the list and the map used to be changed under the thread's feet,
//and a running interpolator's target and curve rewritten while it read them)
...
{
	GenericScopedLock lock(interpLock);
	if (!interpolatorMap.contains(p)) return;
	WeakReference<ValueInterpolator> interp = interpolatorMap[p];
	interpolatorMap.remove(p);
	if (!interp.wasObjectDeleted()) interpolators.removeObject(interp);
}

void Parameter::ValueInterpolator::Manager::removeInterpolationsUsing(Automation* a)
{
	if (a == nullptr) return;
	GenericScopedLock lock(interpLock);
	for (int i = interpolators.size() - 1; i >= 0; --i)
	{
		ValueInterpolator* vi = interpolators[i];
		if (vi == nullptr || vi->automation != a) continue;
		interpolatorMap.removeValue(vi);
		interpolators.remove(i);
	}
}
