...
void DashboardGroupItemUI::paint(Graphics& g)
{
	...
	//no repaint() here : asked from paint(), it queued the next paint at once, so a group repainted every display frame
	//for as long as it was shown (with OpenGL, holding the message thread each time)
