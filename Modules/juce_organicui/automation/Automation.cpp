...
{
	const ScopedLock lock(items.getLock()); //a sequence's play thread follows these links holding it
...
float Automation::getValueAtPosition(float pos)
{
	...
	const float keyLength = k->getLength(); //read once : a key dragged on the message thread moves under a play thread
	if (keyLength <= 0) return k->value->floatValue(); //a key whose next key is not linked yet (mid-insert) : its own value, not a division by zero
	//within the key by construction, unless the next key moved between finding this key and reading its length : then
	//an easing that does not clamp (Linear) extrapolated past the key's value for a frame
	float normPos = jlimit(0.0f, 1.0f, (pos - k->position->floatValue()) / keyLength);
...
float Automation::getNormalizedValueAtPosition(float pos)
{
	...
	AutomationKey* k = getKeyForPosition(pos);
	if (k == nullptr || k->easing == nullptr) return 0;
	const float keyLength = k->getLength();
	if (keyLength <= 0) return (float)k->value->getNormalizedValue(); //see getValueAtPosition
	float normPos = jlimit(0.0f, 1.0f, (pos - k->position->floatValue()) / keyLength);
...
InspectableEditor* Automation::getEditorInternal(bool isRoot, Array<Inspectable*> inspectables)
{
