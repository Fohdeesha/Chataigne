...
float CubicEasing::getValue(const float& weight)
{
	...
	//a sequence's play thread reads this while a key is dragged : the table is swapped whole, never seen half built
	const ScopedLock lock(uniformLUT.getLock());
	const int n = uniformLUT.size();
	if (n < 2) return start.y;

	float indexF = weight * (n - 1);
	int index = jlimit(0, n - 2, (int)floor(indexF));
	float rel = indexF - index;
	float p1 = uniformLUT.getUnchecked(index);
	float p2 = uniformLUT.getUnchecked(index + 1);
...
void CubicEasing::updateUniformLUT(int precision)
{
    ...
    //built aside and swapped in : clearing the table and refilling it in place gave a play thread reading it an empty
    //or half-built table, and so one frame of the easing's start value (a joint target or a DMX value of 0)
    Array<float, CriticalSection> lut;
    lut.ensureStorageAllocated(precision + 1);
    ...
    uniformLUT.swapWith(lut); //takes both locks
}
