...
{
    if (easing != nullptr && easing->type == type) return;

    //a sequence's play thread evaluates this key's easing holding the automation's key list lock : replace it under that
    //lock, or the old one is deleted while it is being evaluated
    BaseManager<AutomationKey>* keys = dynamic_cast<BaseManager<AutomationKey>*>(parentContainer.get());
