...
{
	//built aside and swapped in, as CubicEasing does : a play thread reads the table while a point is dragged
	Array<Point<float>, CriticalSection> lut;
...
Point<float> CubicEasing2D::getValue(const float& weight)
{
	...
	Point<float> p1 = uniformLUT.getUnchecked(index);
	Point<float> p2 = uniformLUT.getUnchecked(index + 1);
