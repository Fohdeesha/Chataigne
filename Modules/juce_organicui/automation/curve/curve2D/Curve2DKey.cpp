...
void Curve2DKey::updateEasing(bool forceRecreate)
{
    ...
    if (easing != nullptr && easing->type == et && !forceRecreate) return;

    //the 2D layer's play thread evaluates the curve holding its key list lock : replace the easing under it
    BaseManager<Curve2DKey>* keys = dynamic_cast<BaseManager<Curve2DKey>*>(parentContainer.get());
    const ScopedLock keysLock(keys != nullptr ? keys->items.getLock() : easingSwapLock);
...
{
    if (easing == nullptr || easing->length <= 0) return viewUIPosition->getPoint(); //was "== nullptr &&" : dereferenced a null easing, divided by a zero length
