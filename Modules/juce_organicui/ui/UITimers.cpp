...
void OrganicUITimers::timerCallback(int timerID)
{
	...
	if (timerMap.contains(timerID))
	{
		...
		uint32 curTime = juce::Time::getMillisecondCounter();
		...
		for (auto& t : targets)
		{
			...
			{
				uint32 timeDiff = curTime - t->lastRepaintTime; //unsigned so the delta stays correct when the counter wraps
				if (timeDiff < (uint32)(getTimerInterval(timerID) / 2)) continue;
			}
...
UITimerTarget::UITimerTarget(int timerID, juce::String _name, bool safeRepaintCheck) :
	...
	lastRepaintTime(juce::Time::getMillisecondCounter()) //not 0 : the counter interpreted as signed goes negative after ~25 days of system uptime, and a 0 baseline would make the safeRepaintCheck throttle skip this target forever
{
