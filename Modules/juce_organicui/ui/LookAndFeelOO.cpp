...
{
    // No drop shadow on popup menus. On Windows, JUCE implements the shadow with four extra
    // top-level layered windows per menu (DropShadower::ShadowWindow), so every submenu hover
    // created five native windows; on systems where CreateWindowEx is expensive (overlay/hook
    // software, large window counts) that made category submenus take hundreds of ms to open.
    return 0;
}
                                                ...
                                                GroupComponent& group)
{
    ...
    p.addArc (x, y, cs2, cs2, MathConstants<float>::pi * 1.5f, MathConstants<float>::pi * 2.0f);
    p.lineTo (x + textX, y);
