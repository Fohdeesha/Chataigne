...
{
	//posted : two REMOVEs of the same item, or a reload, before it runs used to call remove() on a deleted item
	if (!userCanRemove) return;
	WeakReference<Inspectable> weak(this);
	MessageManager::callAsync([weak]() { if (BaseItem* bi = dynamic_cast<BaseItem*>(weak.get())) bi->remove(); });
}
