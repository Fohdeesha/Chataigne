...
public:
	...
	//One thread at a time in the engine, which is not thread safe (a script's update thread, the message thread's
	//events, a sequence's play thread or a launcher running a script command or filter). A plain lock deadlocked when two
	//scripts called into each other from two threads (it was commented out upstream for that), so a caller waits a
	//bounded time and gives up, logged, rather than wait forever.
	juce::CriticalSection engineLock;
	bool enterEngine(int timeoutMs);
	std::atomic<juce::uint32> lastEngineBusyLog{ 0 };
	juce::Time pendingModTime; //a changed file is loaded once it has stayed the same for a timer tick
