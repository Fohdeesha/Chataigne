...
Script::~Script()
{
	...
	//no other thread may be inside the engine when it goes
	if (enterEngine(5000))
	{
		scriptEngine.reset();
		engineLock.exit();
...
bool Script::enterEngine(int timeoutMs)
{
	if (engineLock.tryEnter()) return true;
	const uint32 start = Time::getMillisecondCounter();
	for (int spins = 0; Time::getMillisecondCounter() - start < (uint32)timeoutMs; ++spins)
	{
		if (spins < 50) Thread::yield();
		else Thread::sleep(1);
		if (engineLock.tryEnter()) return true;
	}
	return false;
...
void Script::loadScript()
{
	...
	//the engine is replaced here : only once no other thread is inside it
	if (!enterEngine(5000))
	{
		NLOGWARNING("Script : " + niceName, "The script is busy, reload postponed");
		fileLastModTime = Time(); //the modification check will try again
		return;
	}
	struct EngineExit { CriticalSection& lock; ~EngineExit() { lock.exit(); } } engineExit{ engineLock };
...
var Script::callFunction(const Identifier& function, const Array<var> args, Result* result)
{
	...
	if (!enterEngine(2000))
	{
		...
		if (now - lastEngineBusyLog.load() > 5000)
		{
			lastEngineBusyLog = now;
			NLOGWARNING(niceName, "The script stayed busy on another thread, " << function.toString() << " was not called");
		}
		return var();
	}
	struct EngineExit { CriticalSection& lock; ~EngineExit() { lock.exit(); } } engineExit{ engineLock };

	if (scriptEngine == nullptr) return false; //replaced or gone while waiting
...
void Script::childStructureChanged(ControllableContainer* cc)
{
	...
	if (state == ScriptState::SCRIPT_LOADED && Engine::mainEngine != nullptr && cc == Engine::mainEngine)
	{
		...
		{
			if (enterEngine(2000)) //bounded, like every other entry
			{
...
void Script::timerCallback()
{
	...
	File f = filePath->getFile();
	const Time t = f.getLastModificationTime();
	if (t == fileLastModTime)
	{
		pendingModTime = Time();
		return;
	}

	//a file still being written (a copy in progress) changes again : loading it half written left the script dead until
	//the next change. Loaded once it has stayed the same for a tick.
	if (t != pendingModTime)
	{
		pendingModTime = t;
		return;
