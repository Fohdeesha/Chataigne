...
Inspectable::Inspectable() :
	...
	linkedInspectablesHighlighted(false),
	saveCustomData(true),
...
Inspectable::~Inspectable()
{
	bool unhighlightLinked = isHighlighted || linkedInspectablesHighlighted;
	isHighlighted = false; //unregistering below must not notify this half-destroyed inspectable

	for (auto &i : linkedInspectables)
	{
		if (i.wasObjectDeleted()) continue;
		if (unhighlightLinked) i->setHighlighted(false);
...
{
	linkedInspectablesHighlighted = value;
	for (auto &i : linkedInspectables)
	{
...
void Inspectable::registerLinkedInspectable(WeakReference<Inspectable> i, bool setAlsoInOtherInspectable)
{
	...
	if (linkedInspectablesHighlighted) i->setHighlighted(true);
...
void Inspectable::unregisterLinkedInspectable(WeakReference<Inspectable> i, bool setAlsoInOtherInspectable)
{
	...
	//otherwise a command deleted or retargeted while its mapping is selected or hovered leaves its old module highlighted for good
	if (linkedInspectablesHighlighted && i != nullptr) i->setHighlighted(false);
