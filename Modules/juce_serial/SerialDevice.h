...
public:
	...
	//Locked : the read thread calls it (data) while the message thread adds and removes modules sharing this port, and an
	//unlocked ListenerList keeps its iteration state in a shared vector. No callback waits for the read thread : data
	//callbacks only queue, and the message thread's own calls come once that thread is stopped or not yet reading.
	ListenerList<SerialDeviceListener, Array<SerialDeviceListener*, CriticalSection>> listeners;
