		...
		try
		{
			...
			case SerialDevice::PortMode::COBS:
			{
				while (port->port->available() && port->mode == SerialDevice::PortMode::COBS)
				{
					...
					if (b == 0)
					{
						//decoded into a buffer the size of the frame (a frame over 255 bytes overflowed the fixed one), and an
						//empty frame (two zeros in a row) is skipped (numDecoded - 1 wrapped around to a huge size)
						std::vector<uint8_t> decodedData(byteBuffer.size());
						size_t numDecoded = cobs_decode(byteBuffer.data(), byteBuffer.size(), decodedData.data());
						if (numDecoded >= 1) serialThreadListeners.call(&SerialThreadListener::dataReceived, var(decodedData.data(), numDecoded - 1));
						byteBuffer.clear();
					}
				}
			}
