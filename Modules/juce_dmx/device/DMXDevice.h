...
public:
	...
	juce::CriticalSection sendLock; //one send at a time : the module's thread, a pass-through and the sender thread can all send
	...
	bool sendDMXValues(); //false when nothing was waiting to be sent
	void frameReady(); //the module has filled a whole frame : with a send rate, the sender thread sends it now
