...
public:
	...
	//Reentrant : a multi-channel write (RGB, 16-bit, a range) holds it across all its channels, so a frame is never
	//taken halfway through one, and each channel's own update takes it again on the same thread
	CriticalSection valuesLock;
	Array<uint8> values;

	//The values and the dirty flag together, in one consistent frame. Clearing the flag in the same locked section as
	//the copy : a write landing between the two would otherwise be marked sent and never go out.
	Array<uint8> takeFrame(bool clearDirty);
